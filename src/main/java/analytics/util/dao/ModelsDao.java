...
public class ModelsDao extends AbstractDao {
	...
	public ModelsDao() {
		super();
		tagVariablesCollection = db.getCollection("Models");
...
*/
		...
		if(cursor != null && cursor.count() > 0){
			while(cursor.hasNext()){
				...
				if(modelObj != null){
					...
					tagVariable.setModelName((String) modelObj.get(MongoNameConstants.MODEL_NAME));
	...
	public Map<Integer, Model> getModelNames(){
		...
		DBCursor modelsCursor = tagVariablesCollection.find();
