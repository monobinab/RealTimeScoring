...
/**
 * This code has been updated and modified to fit our needs...
...
public class PseudoWebHDFSConnection implements WebHDFSConnection {
	...
	public synchronized Token generateToken(String srvUrl, String princ, String passwd) {
	...
	protected long copy(InputStream input, OutputStream result) throws IOException {
	...
	private String result(HttpURLConnection conn, boolean input) throws IOException {
		...
		try {
			if(!sb.toString().trim().equals(""))
				jsonResp = new  JSONObject(sb.toString());
			
			jsonResp.append("code", conn.getResponseCode());
			jsonResp.append("mesg", conn.getResponseMessage());
			jsonResp.append("type", conn.getContentType());
			json = jsonResp.toString();
			
		} catch (JSONException e1) {
			System.out.println("Error Occurred while making WwebHdfs Call");
			e1.printStackTrace();
		}
...
/*		Map<String, Object> result = new HashMap<String, Object>();
	...
	public HttpURLConnection getConnection(String path, InputStream is)
	...
	public boolean isExists(String path) throws MalformedURLException, IOException, AuthenticationException {
		boolean isexists = true;
		ensureValidToken();
		String spec = MessageFormat.format("/webhdfs/v1{0}?op=LISTSTATUS&user.name={1}", URLUtil.encodePath(path), this.principal);
		String jsonStr =HttpClientUtils.httpGetCall((new URL(new URL(httpfsUrl), spec)).toString()).toString() ;
		
		if(jsonStr.contains("FileNotFoundException"))
			isexists = false;
